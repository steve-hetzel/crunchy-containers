...
function fill_conf_file() {
	...
	if [[ -v MAX_CONNECTIONS ]]; then
		if [[ $MAX_CONNECTIONS -gt 100 ]]; then
			MAX_CONNECTIONS=100
			echo "MAX_CONNECTIONS > 100, overriding to max at 100 per the STIG."
		else
...
function initialize_primary() {
	...
	echo "Generating server.key and server.crt in PGDATA directory."
	...
	openssl req -new -key $PGDATA/server.key -days 3650 -out $PGDATA/server.crt -x509 -subj '/C=US/ST=Virginia/L=Fairfax/O=crunchydata.com/CN=crunchydata.com/emailAddress=support@crunchydata.com'
	
	cp -p $PGDATA/server.crt $PGDATA/root.crt && cp -p $PGDATA/server.crt $PGDATA/root.crl

	echo "Uncommenting #ssl config lines from postgresql.con to enable SSL."
	sed -i "s/#ssl/ssl/g" $PGDATA/postgresql.conf
	sed -i "s/local   all             postgres                                     trust/local   all             postgres                                     md5/g" $PGDATA/pg_hba.conf
	sed -i "s/host   replication1        replication1                                     md5/hostssl   replication        replication                                     cert/g" $PGDATA/pg_hba.conf
